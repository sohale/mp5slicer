...
def shorten_last_line(line_group, shorten_length):
    ...
    def shorten_vector(end, shorten_length_in_function):  # start is 0,0
        ...
        if end[0] == 0:
            ...
            shorten_length_ratio_x = \
                abs((length_of_end-shorten_length_in_function)*np.cos(angle)/end[0])
    ...
    for index in range(len(line_group.sub_lines)):
        ...
        while shorten_length_copy > 0:
            ...
            if len(each_line) > 2:
                # print('in if')
                each_line = line_group.sub_lines[index]
                vector_from_last_line = [each_line[-1][0] - each_line[-2][0],
                                         each_line[-1][1] - each_line[-2][1]]
                ...
                if isinstance(final_vector, float):
                    ...
                    # tricking the gcode write to go to a new point
                    line_group.sub_lines.insert(index+1, [original_end_point])
                    index_change += 1 # counting the line added to the subline
                else:
                    final_end_point = [final_vector[0] + each_line[-2][0],
                                       final_vector[1] + each_line[-2][1]]
                    original_end_point = line_group.sub_lines[index][-1]
...
def retract_at_point_inside_boundary(line_group,
    ...
    for outer_boundary_index in range(len(line_group.sub_lines)):
        ...
        if retraction_point != None:
            # tricking the gcode write to go to a new point
            line_group.sub_lines.insert(outer_boundary_index+1,
                                        [retraction_point])
            index_change += 1
