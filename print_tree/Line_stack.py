...
class LineStack(object):
    def __init__(self, lines=None):
        if sys.version_info[0] == (3):
            if lines is None:
                self.lines = []
            elif isinstance(lines, LineStack):
    ...
    def intersect_with(self, other):
        if self.is_empty() or other.is_empty():
            return LineStack([])
        return LineStack(inter_layers(self.lines, other.polygons, False))

    # danger this is not union, this is only add two linestack together
    def combine(self, other):
        if self.is_empty() and other.is_empty():
            ...
            return LineStack(lines)

    def difference_with(self, other):
        if other.is_empty():
            return LineStack(self.lines)  # same return format
        if self.is_empty():
            return LineStack([])
        try:
            return LineStack(diff_layers(self.lines, other.polygons, False))
        except:
            raise RuntimeError
    ...
    def offset_point(self, offset_value):
        ...
        sol = lines_offset(points, offset_value)
    ...
    def offset_line(self, offset_value):
        ...
        sol += lines_offset(lines, offset_value)
        return PolygonStack(sol)
...
class SupportLineStack(LineStack):
    ...
    def offset_point(self, offset_value):
        ...
        for each_line in self.lines:
            ...
            if len(each_line) == 1 and \
                    not isinstance(each_line[0], support.Last_point):
                points.append(each_line)
    ...
    def offset_last_point(self):
        ...
        for each_line in self.lines:
            ...
            if len(each_line) == 1 and \
                    isinstance(each_line[0], support.Last_point):
                points.append(each_line[0].return_point_as_new_line())

        sol = lines_offset(points, support.Last_point.offset_value())
        for each_line_index in range(len(sol)):
            sol[each_line_index].append(sol[each_line_index][0])
