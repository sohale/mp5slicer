...
def get_bounding_box_for_tree(shape, ignore_root_matrix):
    # //if(result_bbox === null) {

    if is_single_shape(shape):

        return get_bounding_box_for_single_shape_matrix(shape.matrix)
        ...
        for i in range(0, shape.sons.length):
            ...
            # never ignore the matrix if not root
            sbbox = get_bounding_box_for_tree(shape.sons[i], False)

            # just update, dont apply the matrix
            update_bounding_box_for_sub_bounding_box(result_bbox, None, sbbox)

        ret = update_bounding_box_for_sub_bounding_box(None, shape.matrix, result_bbox)
        return ret
...
def get_bounding_box_for_single_shape_matrix(this_matrix):
...
def get_bounding_box_for_single_shape_matrix_torus(this_matrix):
...
def update_bounding_box_for_sub_bounding_box(result_bbox, this_matrix, input_sbbox):
